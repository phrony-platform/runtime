...
INSERT INTO bundle_versions (id, bundle_id, lock_hash, lock, root_member_version_id)
...
const bundleVersionIDByLabel = `
SELECT bv.id, bv.root_member_version_id
FROM bundle_versions bv
...
func (q *Queries) BundleVersionIDByLabel(ctx context.Context, namespace, name, lockHash string) (BundleVersionLookupResult, error) {
	row := q.db.QueryRowContext(ctx, bundleVersionIDByLabel, namespace, name, lockHash)
	var out BundleVersionLookupResult
	var root sql.NullString
	err := row.Scan(&out.ID, &root)
	if errors.Is(err, sql.ErrNoRows) {
...
SELECT bv.id, bv.lock_hash, bv.root_member_version_id
...
func (q *Queries) ActiveBundleVersion(ctx context.Context, namespace, name string) (ActiveBundleVersionResult, error) {
	...
	var root sql.NullString
	err := row.Scan(&out.BundleVersionID, &out.LockHash, &root)
	if errors.Is(err, sql.ErrNoRows) {
...
SELECT bv.lock_hash, bd.created_at, bd.actor
FROM bundle_deployments bd
INNER JOIN bundle_versions bv ON bv.id = bd.bundle_version_id
INNER JOIN bundles b ON b.id = bd.bundle_id
WHERE b.namespace = $1 AND b.name = $2
...
func (q *Queries) ActiveBundleDeploymentDetail(ctx context.Context, namespace, name string) (ActiveBundleDeploymentDetail, error) {
	...
	err := row.Scan(&out.LockHash, &out.DeployedAt, &out.Actor)
	if errors.Is(err, sql.ErrNoRows) {
